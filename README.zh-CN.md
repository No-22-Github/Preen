...
<p align="center">
  <img src="assets/screenshots/home.png" alt="Preen 首页" width="49%">
  <img src="assets/screenshots/training.png" alt="实时训练曲线" width="49%">
</p>
...
[v1.0.0](https://github.com/No-22-Github/Preen/releases/latest) 已发布,见[下载与安装](#下载与安装)。
...
**现在训练很快。** WKV7 Metal checkpoint kernel 默认开启后,1.5B 模型用约 320 token
的样本跑 540 步,只要 **4 分半左右**(实测 4 分 23 秒,内存约 10GB)。同样一轮在旧的可微
Python ops 循环上要跑约 38 分钟——快约 **8.5×**。见[为什么训练这么快](#为什么训练这么快)。

16GB 内存的机器上,训练集 ctx 可以来到 512 token;再往上的上限还没测。

App 本身也是双语的:完整的简体中文 / 英文本地化,跟随系统语言,后端结构化事件也一并本地化。

实测数据与技术裁决在[工程实测数据](docs/工程实测数据.md)。
...
RWKV-7 的 WKV 递归在 MLX 上有两条等价前向路径。原生 `mlx-lm` 的 kernel 是快但无 VJP 的
Metal 黑盒——梯度会静默断裂,没法训。早期训练只能走可微的 Python `_wkv7_step_ops` 循环:
每个 token 一次 GPU dispatch,ctx=512 就是 512 次。长序列慢到不可行(1.5B × 320 token
单 epoch 约 28 分钟)。

现在训练默认走移植自 [rwkv-metal](https://github.com/RafaelUI/rwkv-metal) 的 Metal
checkpoint kernel:整段递归 forward/backward 各一次 dispatch,经 `mx.custom_function`
注册 VJP,S₀ 的梯度仍能穿透整段递归。

kernel 之上再叠了训练图 `mx.compile` 复用和 bf16 激活直读两轮优化。端到端实测:
**1.5B、约 320 token 样本、540 步、4 分 23 秒**(约 0.49 s/step,内存约 10GB),
对比旧 ops 循环约 4.2 s/step、需约 38 分钟,快约 **8.5×**。序列越长,kernel 消除的
逐 token dispatch 越多,收益越大。数值上末态 loss 与 ops 基线差 0.19%(等价),
完整三轮实验见 [`docs/decision-fast-wkv7.md`](docs/decision-fast-wkv7.md)。

推理路径不变:走 `mlx-lm` 自带 kernel,实测比上游推理 kernel 快约 10%。
`--no-fast-wkv` 可回退旧的 ops 循环用于复现。
...
**核心引擎**是 [ml-explore/mlx-lm](https://github.com/ml-explore/mlx-lm) 里的
`rwkv7.py`(Apple 维护)。**反向传播**全部交给 MLX 自动微分(`mx.value_and_grad`),
没有手写任何反向代码。梯度能不能穿透,取决于前向是否可微(checkpoint kernel 经
`mx.custom_function` 注册 VJP,ops 循环每步自带 VJP),见
[P0 理论指南 §二](docs/P0-理论指南.md)。这个仓库做的是在其上的 state tuning 训练改造,
...
├── fast_wkv7.py                  WKV7 Metal checkpoint kernel (训练 fast path)
...
├── golden/                       推理 golden 快照
└── ...                           多个测试模块 (含 --slow 训练行为断言)

docs/                           文档
├── 快速上手.md                    分步教程,首次微调先看这个
├── decision-fast-wkv7.md         WKV7 Metal kernel 接入决策记录 (三轮实验)
├── RWKV-StateTuner-Roadmap.md    落地路线图
├── P0-理论指南.md                 state tuning 原理
├── 工程实测数据.md                 实测数据 + 技术裁决汇总
...
训练默认走快速的 WKV7 Metal kernel(`--fast-wkv`,chunk 16),加 `--no-fast-wkv`
可回退到慢的可微 ops 循环用于复现。`--cache-limit-gb`(train/eval/chat 通用)默认
`auto`,取物理内存的 25%(16GB 机约 4.3GB);设小能降 RSS,在模型加载前生效。
...
几个可能反直觉的地方:脱离 fla 自写转换器、lr 从 1e-4 起步、训练走 Metal kernel、不依赖 torch。
...
**训练路径为什么换成 Metal checkpoint kernel。** 整段递归一次 dispatch 而非每 token 一次,
经 `mx.custom_function` 注册 VJP 保证 S₀ 梯度穿透。展开见[为什么训练这么快](#为什么训练这么快)。
