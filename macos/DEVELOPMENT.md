...
UI 文案里混用半角标点(`,` `:` `?` 夹在中文里、`()` 夹中文内容)会显得挤,不符合苹果中文规范。扫描脚本:

```bash
python3 scripts/scan_cjk_punct.py macos
```

只扫字符串字面量,且剔除了 Swift 插值 `\(...)`(含嵌套括号),不会把代码本身的括号误报进来。输出按文件列出 `行号 + 建议替换 + 上下文`。
...
- 快捷键提示:`(Esc)`、`(⌘⏎)` —— 半角加空格是 macOS 惯例。
- 括号里是纯英文 / 符号:`格式（.pth → HF safetensors）` 这类夹中文用全角,但纯 `(build)` 版本号保留半角。
- `print(...)` 调试日志、`//` `///` 代码注释 —— 不面向用户,优先级低。

加了新文案后跑一遍复查,把用户可见的清零即可。
