...
detect_import {data_path}               → ok {detection, result?, preview?}
                                          （探测格式 + 前3条渲染预览，不落盘，§4.1 探测步）
                                          schema=unknown 时 result/preview 为空，UI 走手动映射
import {data_path, out_path, turn_policy?}
                                        → ok {jsonl_path, sidecar_path, sha256, record_count, result}
                                          （探测+转换+落盘，§4.1 落盘步；产物可直接喂 train）
shutdown                                → ok 后进程退出 0
```

> **detect_import / import 指令说明（附录 E.5 推断）**：原 Spec §3.3 指令集只列推理类
> 指令，数据导入（§4）原计划走独立 CLI 子进程。实施时为让 Swift 侧统一一条 serve 链路
> （不混 CLI 子进程），把 import 拆成两步加进 serve 协议：detect_import（探测+预览，
> 不落盘，供 UI 确认）+ import（落盘）。两者都是 CPU/IO 操作，不走 busy 锁（无生成）。
> detect_import 复用 engine 持有的 tokenizer 做 preview_records 渲染（token 级 mask 边界）。
...
> 落地状态（2026-07，§3 已实施）：全部通过。详见各条勾注与实测证据。

- [x] a. 每个请求恰好收到一个终结事件（ok 或 error），乱序/并发请求下 id 对应无误
      _实测：`test_serve_handlers.py` 22 例覆盖全指令路由 + id 透传；
      每个请求的流式事件（text_chunk* / turn_end）与终结事件（ok/error）
      都带回原 id；`_send_and_collect` 断言每请求恰好一个终结事件。_
- [x] b. 生成中途 abort，300ms 内收到 aborted 终结，进程可继续服务下一请求
      _实测：`test_serve_e2e.py::test_abort_within_300ms`（spawn 真实进程），
      发 send → 等首个 text_chunk 确认进入生成循环 → 发 abort → 收 aborted；
      abort 经读线程 `threading.Event.set()` → `ChatSession.abort_checker` →
      `generate(should_abort=)` 每步检查，延迟到下一个 MLX step 边界（~50-200ms）。
      abort 后 hello 仍正常服务。_
- [x] c. 向 stdin 灌 fuzz 垃圾行（非 JSON / 超长行 / 未知 cmd），进程存活且逐行回 error
      _实测：`test_serve_e2e.py::test_fuzz_garbage_lines_survive`（spawn 真实进程，
      灌非JSON/超长畸形/未知cmd/缺cmd/非object 五类垃圾）+ 快测
      `test_malformed_does_not_crash_repeatedly`；进程存活且后续 hello 正常。
      handle_line 全包 try/except → error{bad_request}，不变式铁律守住。_
- [x] d. preview ab=true 的两路 result 与现有 CLI `preview --ab --json` 数值一致（同 seed）
      _实测：`test_serve_e2e.py::test_preview_ab_matches_cli`，raw 模板 + seed=42 +
      temp=0 贪心，serve 路径与 CLI 子进程路径的 with_state.text 逐字相等。_
- [x] e. stdout 全程无非 JSON 行（管道 `| jq .` 通读不报错）
      _实测：`test_serve_e2e.py::test_stdout_all_json`，读线程把非 JSON 行记成
      `{"__parse_error__": line}`，整轮交互后断言无此类行；
      人类日志一律走 stderr（`_log` helper，§3.2 惯例）。_
...
> 落地状态（2026-07，§4 已实施）：全部通过。详见各条勾注与实测证据。

- [x] a. 四类格式各取一个真实 HF 数据集样本文件，探测全部正确命中
      _实测：`test_importer.py` + `tests/fixtures/import/` 四类 fixture
      （alpaca_sample.jsonl / sharegpt_multiturn.jsonl / messages_chatml.jsonl /
      bare_qa.jsonl），`detect_schema` 各命中正确 schema，confidence=1.0。_
- [x] b. 探测失败路径：故意喂 DPO 格式（chosen/rejected），走到手动映射而非误判
      _实测：`test_detect_dpo_falls_to_unknown`，DPO 格式（有 prompt 但
      chosen/rejected 不在 RESPONSE_ALIASES）→ schema=unknown；
      `test_convert_unknown_raises` 验证 convert 抛错让 UI 走手动映射。_
- [x] c. ShareGPT 多轮文件，`first` 与 `all` 两种策略的产物行数与人工计数一致
      _实测：`test_convert_sharegpt_first_policy`（3 条原始 → 3 条样本，首对）
      + `test_convert_sharegpt_all_policy`（3 条原始含 2+1+2 对 → 5 条样本）；
      system 消息丢弃计数（`dropped_system==1`）同步验证。_
- [x] d. 导入产物直接喂给 train 命令可跑通（端到端冒烟）
      _实测：`test_importer_e2e.py::test_imported_jsonl_feeds_train`（--slow），
      alpaca fixture → import（instruction 模板）→ train 1 epoch →
      state.npz 产物可被 state-info 读回（24 层）。数据分流：service.run_training
      检测 `.import.json` sidecar → 走 `load_standard_jsonl` + `inspect_standard_jsonl`。_
- [x] e. import.json 的 hash 可复现（同文件两次导入 hash 相等）
      _实测：`test_import_json_hash_reproducible`，同文件两次 import_dataset，
      sidecar 的 source.sha256 相等 + record_count 相等。
      sha256 由 `metadata.file_sha256` 计算（与训练产物 meta.json 同源）。_
...
>
> **#4/§4-backend 落地状态（2026-07-12）**：
> - **#4 serve 实现（§3）已完成**：`src/statetuner/serve.py`（ServeSessionManager +
>   ServeProtocol，读线程 + `threading.Event` abort）+ `cli.py serve` 子命令。
>   §3.6 验收 a/b/c/d/e 全部通过（test_serve_handlers.py 22 例快测 +
>   test_serve_e2e.py 4 例真实进程慢测）。abort 经 `ChatSession.abort_checker` →
>   `generate(should_abort=)` 透传，延迟到下一个 MLX step 边界。
> - **§4 数据导入 backend 已完成**：`src/statetuner/importer.py`（探测 + 转换 +
>   产物 + 预览渲染）+ `data.py load_standard_jsonl` + `inspection.inspect_standard_jsonl` +
>   `cli.py import` 子命令。§4.5 验收 a/b/c/d/e 全部通过（test_importer.py 19 例快测 +
>   test_importer_e2e.py 1 例 train 冒烟慢测）。数据分流：service.run_training 检测
>   `.import.json` sidecar → 走标准 loader，遗留数据零改动。
> - **§4 的 UI 集成（拖入/映射确认/渲染预览）属 #7**，待 Swift spike 后接入；
>   backend service 层已就绪，UI 直接调用 `importer.detect_schema/convert/preview_records`。
...
## 附录 E：§3/§4 落地裁决（2026-07-12，serve 协议 + 数据导入实施时）

§3（serve 会话协议）与 §4（数据导入）落地时，Spec 只定义了行为契约，未规定部分内部
实现机制。本附录记录 4 处实施推断（含 1 处违反 Spec 明文的 bug 修复），可追溯。

### E.1 abort 双路径：读线程内联 + 主线程同步兜底（§3.3 推断）

**Spec §3.3 契约**：`abort → 被中断请求收 error{code:aborted} → ok`，只规定行为。

**推断**：abort 有两条处理路径——
- **读线程内联**（`_maybe_handle_abort_inline`）：读线程 readline 到 abort 指令时，
  不进队列，直接 `threading.Event.set()` 并回 ok。**必须绕过队列**，因为主线程正在
  跑 generate（阻塞在 for step 循环），只有读线程能并发接收 abort 信号。
- **主线程同步兜底**（`_dispatch` 的 abort 分支）：测试直接调 `handle_line`、
  或无在跑生成时的幂等回 ok。

**理由**：如果 abort 也进队列排队，主线程在 generate 期间根本读不到它 → abort 永远
无法中断当前生成。Spec 说"中断当前生成"隐含了"abort 必须能并发于 generate 生效"，
实现上必须有绕过队列的快速路径。这是 Spec 未写但实现必需的机制。

### E.2 should_abort 透传链：serve → ChatSession.abort_checker → generate（§3.3 推断）

**Spec §3.3 契约**：abort 能中断生成，**未规定中断信号怎么从协议层传到 generate 循环**。

**推断**：三层透传——
- `InferenceEngine.generate` 新增 `should_abort: Optional[Callable[[], bool]]` 参数，
  for step 循环开头检查，True 则抛 `GenerationAborted`。默认 None（零影响现有调用方）。
- `ChatSession` 新增 `abort_checker` 属性（默认 None），`_handle_single` 的三处
  generate 调用透传 `should_abort=self.abort_checker`。
- `ServeProtocol._handle_send` 在生成前 `session.abort_checker = self._abort_event.is_set`。

**理由**：中断信号的内部 plumbing 是实现细节，Spec 不该规定。选择属性注入
（`session.abort_checker = ...`）而非改 `ChatSession.handle` 签名，是为了不影响
现有 CLI/chat 命令的调用（它们不需要中断能力）。`should_abort` 默认 None 保证
CLI/service/ChatSession 的既有调用零改动。

### E.3 数据分流：sidecar 检测选 loader（§4.3 推断）

**Spec §4.3 契约**：导入产物字段名是 `{prompt, response}` 或 instruction 三字段版，
验收 d 要求"产物直接喂 train 可跑通"。**未规定 train 怎么同时支持遗留数据
（instruction/output 字段）和新产物**。

**推断**：`service.run_training` 检测数据文件旁有没有 `<name>.<suffix>.import.json`
sidecar——有则走 `load_standard_jsonl` + `inspect_standard_jsonl`（新标准字段），
无则走遗留 `load_qa_dataset` + `inspect_data`（instruction/output 字段）。

**理由**：importer 产物必然带 sidecar（§4.3 末段强制要求写 sidecar），这是个可靠的
判别信号，不需要让用户显式声明数据类型。两条路径都有完整的截断检查，遗留数据文件
零改动。held-out（test_data）目前只支持遗留格式；importer 产物的 held-out 若有需求
后续加同款分流。

### E.4 busy 错误码 bug 修复（§3.5 明文，实施时违反 → 已修）

**Spec §3.5 明文**：5 个独立错误码 `bad_request / not_found / busy / aborted / internal`。

**实施时违反**：初版 `serve.py` 用基于 message 字符串匹配的 `_classify_protocol_error`
推断错误码——busy 消息（`"busy: 已有生成进行中"`）被匹配成 `bad_request` 发出，
**busy 错误码永远不会出现**。这是设计草率（用字符串匹配而非结构化 code 字段），
不是 Spec 模糊。

**修复**：给 `ProtocolError` 加结构化 `code` 字段（默认 `bad_request`，由 raise 处
显式指定 `not_found` / `busy`），废止 `_classify_protocol_error`。各 raise 点：
`get_session` 带 `code="not_found"`，两处 busy 检查带 `code="busy"`，其余参数校验
保持默认 `bad_request`。

**教训**：Spec 明文规定的枚举值（错误码、状态码等），实现必须用结构化字段承载，
不能用字符串匹配兜底——后者一旦新增枚举值（或 message 措辞变化）就静默失效。
初版测试 `test_send_busy_rejects_concurrent` 断言 `code == "bad_request"` 是错的
（被实现带歪），修复后改为断言 `code == "busy"`。

### E.5 import 指令加入 serve 协议（§3.3/§4.1 推断）

**Spec 原计划**：§4 数据导入走独立 CLI 子进程（`statetuner import`），§3 serve 协议
只列推理类指令（hello/new_session/send/abort/preview/...）。Spec §3.3 指令集原表
不含 import 相关指令。

**推断变更**：把 import 拆成两步加进 serve 协议——
- `detect_import {data_path}` → 探测格式 + 前3条渲染预览（不落盘，§4.1 探测步）
- `import {data_path, out_path, turn_policy?}` → 探测+转换+落盘（§4.1 落盘步）

**理由**：原计划 Swift 侧要混用两条链路——推理走 serve 常驻进程，导入走 CLI 一次性
子进程。混用让 SidecarClient 要管两种进程生命周期（常驻 vs 一次性）、两种输出协议
（serve JSON lines vs CLI stdout/stderr）。用户裁决：统一一条 serve 链路，不混 CLI
子进程，SwiftUI 代码不混 Python 调用风格。

**两步拆分而非一步的理由**：§4.1 流程是"拖入 → 探测确认 → 预览 → 落盘"，UI 需要
在落盘前展示探测结果给用户确认（schema 对不对、前3条渲染预览）。detect_import 供
UI 确认步（不落盘），import 供确认后落盘。也支持跳过确认直接 import（一键导入场景）。

**设计细节**：
- 两者都是 CPU/IO 操作（探测/读写文件），不走 busy 锁（无生成，不与 send 冲突）。
- detect_import 复用 engine 持有的 tokenizer 做 `preview_records` 渲染（token 级 mask
  边界，§4.4 颜色分界用）。这也是 import 指令必须在 serve 进程内（而非纯协议层）
  的原因——tokenizer 已随模型加载常驻。
- 探测失败（schema=unknown）时 detect_import 不报错，detection 原样返回（result/preview
  为空），UI 据此走手动字段映射；import 则报 bad_request（附原因）。
- sidecar 落盘仍剥掉 detection.sample（来源文件已有，sidecar 只做元数据追溯）；
  sample 只在 detect_import 的响应里序列化（供 UI 展示前3条原文）。

**CLI `import` 子命令保留**：与 serve `import` 指令共用同一 service 层
（`importer.import_dataset`），CLI 命令不删（脚本化场景仍有用）。两条入口等价。
