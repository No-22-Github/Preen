...
| 320(nekoqa_320) | 4.1s | **27 分钟** | 4.2 分钟 | **6.67×** | 勉强(超时风险) |
...
配置:1.5B g1g,lr=1e-4,2 epoch(400 步),seed=42。ops 基线由用户手动跑(27 分钟)。
...
- chunk=8 在长序列(512 token)下的单独对照(当前 chunk=16 已足够)。
