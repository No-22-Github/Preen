...
但这一结论来之不易——经历了一轮**因参数误用导致的否决性结果**,二轮反转。
两个独立因素(lr、chunk)必须同时正确,任一错位即分叉。详见下文。
...
1. **lr=0.01 对梯度误差极度敏感**。高 lr 下,微小的梯度差异经优化器反馈循环被
   指数放大,butterfly effect 式分叉。
2. **chunk=32 在真实训练分布下反向精度不够**。checkpoint 每 32 步存一次,
   反向重构跨越 32 步的误差累积。

**任一因素单独存在都不致命:**
- chunk=32 + 低 lr(1e-4):loss 差 8.27%,偏但可控(第二轮 fast c32);
- 小 chunk + 高 lr:未单独测,但低 lr 已足够,预期也偏。

**两者叠加 → 166% 分叉;两者都修正 → 0.15% 吻合。**
