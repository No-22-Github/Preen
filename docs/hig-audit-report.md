...
> **状态(2026-07-14 更新):** 全部 **10 个 Major 已修复**(见各页 Major 条目下的「✅ 已修复」注脚 + 文末汇总表)。Minor / Nit 未处理,留档备查。
...
> ✅ **已修复(2026-07-14)** — `TrainingConfigView.swift:31-56` 改用 `Button { expanded.toggle() }` 包裹整行热区,并补 `.accessibilityLabel("训练参数")` + `.accessibilityValue(已展开/已折叠)` + `.accessibilityHint`,VoiceOver 识别为按钮、键盘可聚焦。
...
> ✅ **已修复(2026-07-14)** — `TrainingRunningView.swift:81` 加 `ProgressView(value: store.progress)`,totalSteps>0 时显示 determinate 进度条。
...
> ✅ **已修复(2026-07-14)** — `TrainingChartView.swift:38-67` 引入 `ChartLineLegend`,loss 图(Raw/EMA)与内存图(RSS/Swap)各列均带「色 + 线宽 + 文字标签」三通道图例。
...
小计:Blocker 0 / Major 3(✅ 均已修复)/ Minor 12 / Nit ~10。
...
> ✅ **已修复(2026-07-14)** — `ChatPanel.swift:226,326-403` 引入 `ChatScrollPositionObserver(isNearBottom:tolerance:80)`,仅当读者贴近底部(tolerance 80pt)时才 `scrollTo("chat-bottom")`;用户上滑读历史时不再被强拉回。
...
> ✅ **已修复(2026-07-14)** — `ChatPanel.swift:48-49,287-303` 新增常驻内联横幅 `chatErrorBanner(_:)`:只要 `store.lastError` 非空就在消息流上方显示,带关闭按钮调 `store.clearLastError()`。
...
> ✅ **已修复(2026-07-14)** — `StartupLogSheet.swift:97` 改用语义色 `Color(nsColor: .textBackgroundColor)`,深浅模式自动适配。
...
> ✅ **已修复(2026-07-14)** — `ChatPanel.swift:279` 加常驻弱提示「回答由本地 AI 模型生成,可能不准确,请核实重要信息。」
...
小计:Blocker 0 / Major 4(✅ 均已修复)/ Minor 16 / Nit 2。
...
> ✅ **已修复(2026-07-14)** — `TrainingHistoryView.swift:20-24` 零记录分支走独立 `noRunsView`;`:145-151` 筛选无结果态显示「没有符合筛选条件的记录」ContentUnavailableView,消除「空列表却提示选择一条」的矛盾。
...
小计:Blocker 0 / Major 1(✅ 已修复)/ Minor 13 / Nit 3。
...
> ✅ **已修复(2026-07-14)** — `ToolboxView.swift:481-490` 预览改为分段块结构:每段用 `datasetPreviewTextSection(title:text:)` 渲染,「输入前缀」/「训练目标」各有独立标题标签 + `Divider` 分隔,不再仅靠蓝绿着色,VoiceOver 能识别段边界。
...
小计:Blocker 0 / Major 1(✅ 已修复)/ Minor 8 / Nit 1。
...
> ✅ **已修复(2026-07-14)** — `ContentView.swift:129-141` 「需要选择模型」空态内嵌 `Button("选择模型…") { pickModel() }` 直接触发模型选择,辅助文案改为「也可以使用窗口右上角的模型菜单」,与实际控件位置一致。
...
小计:Blocker 0 / Major 1(✅ 已修复)/ Minor ~5 / Nit 0。
...
> **更新(2026-07-14):** 10 个 Major 全部修复,每条 Major 下方均补了「✅ 已修复」条目,附 `file:line` 证据。Minor / Nit 暂未处理,留作后续。
...
~~先清 10 个 Major(尤其页4 预览可访问性、页2 自动滚动/错误可见性/AI 披露、两处空态文案)~~ → **已完成(2026-07-14)**,10 个 Major 全部修复。

后续建议按上述 6 类系统性问题批量整改 ~54 个 Minor。整体没有 Blocker,基础架构(语义色、破坏性确认流、状态双通道、Liquid Glass 版本门控)扎实,剩余问题集中在可访问性细节、空态/错误反馈完整性与 macOS 键盘/菜单栏惯例。
