...
### 训练
- **lr 默认 0.0001,lr_floor 默认 0.00001**(不是 RWKV-PEFT 老教程的 1.0;1.0 会爆炸)。显式传参的历史实验仍按各自配方解读。
...
### 训练性能画像(2026-07-19 插桩实测,M5 + 1.5B + ctx320)

> 工具在 `perf/train-profiler` 分支(未合并 main,按需切换重跑)。完整报告 `experiments/perf/REPORT.md`(本地留档)。下面是**已验证为真**的事实,修正了几条旧错误数据。

- **训练单步时间分布**(400 步实测):稳态每步 480ms,**T_eval(GPU Metal)占 99.0%**,T_setup(Python)0.96%,T_graph(lazy 图构建)0.0002%。MLX lazy 语义把 `loss_and_grad + clip + apply_gradients` 全压成图构建,400 步累计仅 0.4ms——Python 端没有可挤的水分。
- **T_eval 内部组件**:forward 38% / backward(VJP)61% / optimizer+loss 1%。**wkv7 Metal kernel fwd+bwd 占 step 35.6%(170ms)是单一最大组件**;wkv7 反向是正向的 5.1×(checkpoint 重算 + 6 个梯度输出)。
- **ctx 扫描拟合**:`T(L) = 48 + 1.263·L ms`(R²=0.992,线性,二次项不显著)。fixed cost 48ms(占 10%,launch + dispatch overhead),per-token 1.26ms。**ctx 320→192 省 162ms(34% of step)**。配合内存的 `3.65 + 0.0057·L GB`,ctx 对时间和内存都是线性显著影响。
- **GEMM roofline(M5 bf16)**:训练 shape `(320,2048)@(2048,2048)`(r/k/v/o_proj)只跑满 **40% roofline(4.8 TFLOP/s)**,`(320,2048)@(2048,8192)`(FFN)跑满 **74%(9.0)**,roofline 上限 12.2(4096³)。原因是 **M=320 太小 GPU 利用率低**(算术强度 243 FLOP/byte 远超平衡点,不是 bandwidth-bound,是 grid 不够大 SM 空闲)。bf16/fp32 = 1.5×(理论应 8×)→ **MLX 未利用 M5 matrix hardware**,是框架层优化空间。
- **dtype 流全程 bf16,无 fp32 污染**:1.5B 上 335 次 Linear 全 bf16→bf16,wkv7 输入 bf16/state fp32/输出 y 强制 cast 回 bf16。state 是唯一故意 fp32 的张量(保梯度精度)。**attn 投影 vs FFN 的 TFLOP/s 差距不是 dtype 污染,是形状差异**(FFN 大 GEMM 接近 roofline,attn 是多个小 GEMM + LoRA 利用率低)。
- **Metal dispatch 不是瓶颈**:full_step(compile)130 dispatch/步,每 dispatch 平均 3.7ms,**远超 launch overhead(~10µs)370×**。mx.compile 已有效融合 LoRA 细长小矩阵和 elementwise op。时间在 kernel 计算本身,不在 dispatch 启动。
- **batch=2 端到端实测仅 +10%**(TR1 删 batch 决策仍成立):单样本等效时间从 478ms 降到 435ms(加速 1.10×),batch=2/batch=1 时间比 1.82×,峰值内存 5.5→7.3 GB。微基准预测的 +20-30% 在端到端里被打折——**wkv7 kernel 是瓶颈(占 step 35.6%),而它从 batch 维度的收益有限(1.82×)**;GEMM 虽受益大(每样本 0.43×)但 GEMM 不是瓶颈。**batch 救活了非瓶颈组件,没救活瓶颈**。教训:组件级优化预测必须用端到端实测验证。
- **内存峰值构成**(1.5B + ctx320,峰值 6.50GB):模型权重 3.46GB(53%)+ 训练 activation 2.97GB(46%)+ baseline 0.07GB(1%)+ **state params 12.6MB(0.19%)**——state tuning 的根本优势:可训参数比模型权重小 280 倍。
...
### macOS SwiftUI(Tahoe 按钮形态)
- **按钮是胶囊还是圆角矩形,由按钮高度决定,不是 buttonStyle。** macOS 26 (Tahoe) 的 Liquid Glass 走 concentric corners:圆角半径随高度放大,半径大到接近「高度/2」就被磨成整颗胶囊;标准高度就是圆角矩形(比 macOS 15 时代圆角更大,但仍是矩形)。
- **实验实测矩阵(2026-07-16,工具箱·模型转换页一字排开 4 变体验证):**

  | | `.controlSize(.large)` | 标准高度(不加) |
  |---|---|---|
  | `.borderedProminent` | **胶囊** + 跟随主题色 | **圆角矩形** + 跟随主题色 |
  | `.bordered` | **胶囊** + 中性灰 | **圆角矩形** + 中性灰 |
...
- **后端与 App 分口径(经用户裁决,2026-07-16)**:Python 后端、CLI、事件、日志、实验与训练判据统一使用十进制 GB(÷10⁹);macOS App 内面向界面的进程内存、swap、设备容量、working set、图表与压力比例统一使用 GiB 数值(÷1024³),但产品界面单位统一显示为 `GB`。详见下方「内存单位」节。
...
- **产品 CLI 已接入 `--cache-limit-gb`**(train/eval/chat),**缺省 = `auto`(物理内存 × 25%,16G 机 ≈ 4.3G,c4G 同档)**。可显式传 GB 数(如 `--cache-limit-gb 4`)或 `auto` 覆盖。该参数必须在 load_model 前生效,`cli.py` 的 `_apply_cache_limit` helper(`runtime.apply_cache_limit`)已保证时序(set_cache_limit 在 load_model 之前)。
...
- **ctx_len 对内存有显著线性影响**(2026-07-19 perf 插桩实测修正,推翻旧观察):1.5B 上 `footprint = 3.65 + 0.0057·L GB`(R²=0.9996),ctx 64→512 实测 4.02→6.59 GB,ctx 192→512 增量 1.83 GB。旧观察「ctx 512→192 RSS 几乎不变」是当时的 `ru_maxrss` / `mx.get_peak_memory` 漏报口径造成的(这两个口径漏 IOKit 映射,见内存口径节)。但**大头仍是与 L 无关的 fixed 部分**(3.65 GB = 模型权重 + state 提升),ctx 只是叠加线性增量。
- 0.4B 和 1.5B 层数都是 24,state 参数量接近;内存问题两者同源。

排查技巧:`PYTHONPATH=src .venv/bin/python -c "..."` 里逐组件 `load_model` / `make_state_params` / `value_and_grad`,每个之后用 `statetuner.runtime.memory_report()` 读真实 `phys_footprint`(== Activity Monitor 内存列,含 Metal IOKit)。**不要用 `resource.getrusage(RUSAGE_SELF).ru_maxrss`**——它漏报 IOKit 映射;更不要用 `mx.get_peak_memory()`——只算 MLX allocator。
...
## ⚠️ 内存单位:后端 GB;macOS App 用 GiB 数值但显示 GB

同一字节数在两端会有不同数字:`12713115648 bytes` 在后端/日志中是 `12.71 GB`,在 macOS App 中按 GiB 换算后显示为 `11.84 GB`。这是经用户裁决的产品约定,不是可以跨端直接比较的同一数值。

**统一规矩:**
- **Python 后端、CLI、事件、日志、实验报告、缓存上限、训练/削顶判据**一律 GB(÷10⁹)。后端字段 `_gb` 必须放十进制 GB,换算写 `x / 1e9`;`--cache-limit-gb` 也继续按十进制 GB 解释。
- **macOS App** 的进程 footprint、swap、设备容量、working set、内存图表、悬停值与压力比例一律使用 GiB 数值(`bytes / 1024³`)。Swift 内部字段优先以 `GiB` 命名,避免把二进制值误传回后端。
- **App 的可见单位仍统一写 `GB`**,不要显示 `GiB` 或缩写 `G`。例如 16GiB 物理内存在 App 中显示 `16 GB`,12,713,115,648 bytes 的进程内存显示 `11.84 GB`。
- `doctor_report()` 新协议只提供 `memory_size_gb`/`working_set_gb`;macOS App 从十进制 GB 还原 bytes 后再换算为 GiB。Swift 可保留对历史 `_gib` 字段的可选解码兼容,但新后端不得继续产出或记录 GiB。
- **比例和阈值必须在各自端内同口径比较**:后端 active/cache/working_set 全部用 GB;App footprint/capacity/working_set 全部用 GiB。禁止拿 App 的 `11.84 GB` 展示值与后端日志的 `12.71 GB` 直接做加减或判线。
- `mx.metal.device_info()` 返回 bytes:进入后端字段 `_gb` 时 `/1e9`;进入 App 展示数据时 `/1024³`。历史实验脚本与归档数据维持原口径,不要回写。
...
- 估算训练时长决定执行方式:
  ...
  - **0.4B @ 200条 @ ctx192**:~8 分钟(步数不变)。
  - **1.5B @ 200条 @ ctx320**:~3.2 分钟(2026-07-19 perf 插桩实测,稳态单步 480ms × 400步)。
  - **ctx_len 对单步耗时是线性显著影响**(2026-07-19 实测修正,推翻旧观察):`T(L) = 48 + 1.263·L ms`(R²=0.992),ctx 320→192 单步省 162ms(34% of step)。旧表述「ctx 只影响单步内存不显著影响速度」是错的——ctx 对时间和内存都有显著线性影响。
