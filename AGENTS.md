...
├── data.py              数据管线 (load_dataset 翻译 / load_qa_dataset QA)
├── templates.py         ★ 格式模板单一事实源(P0_BARE / NEKO_QA)
├── train.py             训练循环(产品化)
...
tests/                   回归测试(改 src 必跑)
tools/                   模型转换(convert_rwkv7_to_hf.py)
experiments/p0_translate/  ★ P0 历史归档,不要动
train_data/              NekoQA_10k/ + translate/
```
...
- `data.py` 的 `encode_template_sample`(通用)是基础,`encode_sample`(P0 翻译 cn/en)和 `load_qa_dataset`(NekoQA q/a)都派生自它。新任务加 loader,别 fork。
...
# 生成/预览(--template nekoqa|p0_bare|raw)
