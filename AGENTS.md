...
- **全仓内存计算统一 GB(÷10⁹)**;仅 macOS 环境容量展示允许明确标注的 GiB 字段(见下方「内存单位」节)。
...
## ⚠️ 内存单位:计算统一 GB(÷10⁹);环境容量展示允许 GiB
...
**统一规矩:**
- **训练判据、事件、图表、working set 与运行时内存一律 GB(÷10⁹)**。换算统一写 `x / 1e9`,不要 `/1024³`。
...
- **唯一例外(经用户裁决,2026-07-15):macOS 环境信息中的容量展示**可用 `memory_size_gib` 与 `working_set_gib = bytes / 1024³`,明确标 `GiB`(如 `16 GiB`、`11.84 GiB`),用于让同一页面的设备总内存和 MLX 建议上限保持一致。`doctor` 同时保留 `memory_size_gb`、`working_set_gb` 兼容字段;GiB 字段只供环境窗口、命令行人类可读摘要和 Issue 诊断摘要展示,不得参与训练、缓存、比例或削顶计算。
- 除上述 macOS 环境容量展示外,报告/表头数字统一标 "G"(意指 GB),不要写 "GiB"。
